...
from src.diamond_layer1.database_manager import DiamondDatabaseManager
...
class DiamondLayer:
    ...
    def initialize_database(self):
        ...
        try:
            ...
            # Ensure predictions table exists (for DL outputs)
            if hasattr(self.db_manager, 'create_diamond_predictions_table'):
                self.db_manager.create_diamond_predictions_table()
    ...
    def _prepare_advanced_data(self, cotations: pd.DataFrame, indices: pd.DataFrame) -> Tuple[Dict[str, pd.Series], pd.DataFrame]:
        """
        FIXED: Prepare data specifically for advanced models
        
        PURPOSE: Creates properly formatted data for GARCH, VAR, and other advanced models
        """
        # 1. Prepare cotations data with sector mapping
        if 'sector' not in cotations.columns:
            cotations = tests.validator._prepare_data(cotations, is_cotations=True)
        
        # 2. Create sector-specific price series for GARCH
        sector_prices = {}
        sector_returns = {}
        
        for sector in cotations['sector'].unique():
            if sector != 'UNKNOWN':
                sector_data = cotations[cotations['sector'] == sector]
                if len(sector_data) >= tests.validator.min_garch_obs:
                    # Create price series
                    price_series = sector_data.set_index('seance')['cloture'].sort_index()
                    sector_prices[sector] = price_series
                    
                    # Create returns series for GARCH
                    returns_series = price_series.pct_change().dropna()
                    if len(returns_series) >= tests.validator.min_garch_obs:
                        sector_returns[sector] = returns_series
        
        # 3. Create aligned index series for VAR
        indices_pivot = indices.pivot(index='seance', columns='lib_indice', values='indice_jour')
        
        # Select indices with sufficient data (at least 80% non-null)
        min_obs = len(indices_pivot) * 0.8
        valid_indices = []
        for col in indices_pivot.columns:
            if indices_pivot[col].dropna().shape[0] >= min_obs:
                valid_indices.append(col)
        
        # Take top indices for VAR analysis
        selected_indices = valid_indices[:5] if len(valid_indices) >= 5 else valid_indices
        var_data = indices_pivot[selected_indices].dropna()
        
        self.logger.info(f"Prepared {len(sector_returns)} sector return series for GARCH analysis")
        self.logger.info(f"Prepared {len(selected_indices)} indices for VAR analysis")
        
        return sector_returns, var_data

    def _run_garch_analysis(self, cotations: pd.DataFrame) -> Dict[str, Any]:
        """
        FIXED: Run GARCH volatility analysis with proper data preparation
        ...
        # FIXED: Use proper data preparation
        sector_returns, _ = self._prepare_advanced_data(cotations, pd.DataFrame())
        
        garch_count = 0
        for sector, returns in sector_returns.items():
            try:
                # FIXED: Additional data quality checks
                if len(returns) < tests.validator.min_garch_obs:
                    self.logger.debug(f"Skipping {sector} - insufficient data: {len(returns)} < {tests.validator.min_garch_obs}")
                    continue
                
                # FIXED: Check for sufficient variation
                if returns.std() < 1e-6:
                    self.logger.debug(f"Skipping {sector} - insufficient variation")
                    continue
                ...
                if 'error' not in garch_result:
                    ...
                    garch_count += 1
                    self.logger.debug(f"GARCH analysis successful for {sector}")
                else:
                    self.logger.warning(f"GARCH analysis failed for {sector}: {garch_result['error']}")
    ...
    def _run_var_analysis(self, indices: pd.DataFrame) -> Dict[str, Any]:
        """
        FIXED: Run Vector Autoregression analysis with proper data preparation
        ...
        try:
            # FIXED: Use proper data preparation
            _, var_data = self._prepare_advanced_data(pd.DataFrame(), indices)
            
            if len(var_data) >= 50 and len(var_data.columns) >= 2:
                # FIXED: Use advanced validator for VAR analysis
                ...
                if 'error' not in var_result:
                    ...
                    self.logger.info(f"VAR model fitted with {len(var_data.columns)} indices", 
                                    extra={'phase': 'ADVANCED_ANALYSIS'})
                else:
                    self.logger.warning(f"VAR analysis failed: {var_result['error']}")
            else:
                self.logger.warning(f"VAR analysis skipped - insufficient data: {len(var_data)} rows, {len(var_data.columns)} columns")
        ...
        try:
            ...
            for stock in top_stocks:
                ...
                try:
                    ...
                    training_results = self.model_builder.train_model(cnn_lstm, X, y, epochs=20, model_name="cnn_lstm")
                    # Get final loss from returned training results dict
                    final_loss = float(training_results.get('final_metrics', {}).get('val_loss',
                                      training_results.get('final_metrics', {}).get('train_loss', 0.0)))
                    ...
                    # Generate and attach predictions for the last 20% split
                    try:
                        split_idx = int(len(X) * 0.8)
                        X_test = X[split_idx:]
                        pred_payload = self.model_builder.predict_with_model("cnn_lstm", X_test)
                        self.logger.debug(f"CNN-LSTM prediction payload keys: {list(pred_payload.keys()) if isinstance(pred_payload, dict) else 'N/A'}", extra={'phase': 'MODEL_TRAINING'})
                        if isinstance(pred_payload, dict) and 'predictions' in pred_payload:
                            model_results[f'{stock}_cnn_lstm_predictions'] = pred_payload
                        elif isinstance(pred_payload, dict) and 'sample_predictions' in pred_payload:
                            # Fallback acceptance
                            model_results[f'{stock}_cnn_lstm_predictions'] = {
                                'model_name': pred_payload.get('model_name', 'cnn_lstm'),
                                'predictions': pred_payload.get('sample_predictions', [])
                            }
                    except Exception as e:
                        self.logger.warning(f"CNN-LSTM prediction failed for {stock}: {str(e)}", extra={'phase': 'MODEL_TRAINING'})
                ...
                try:
                    ...
                    training_results = self.model_builder.train_model(transformer, X, y, epochs=20, model_name="transformer")
                    final_loss = float(training_results.get('final_metrics', {}).get('val_loss',
                                      training_results.get('final_metrics', {}).get('train_loss', 0.0)))
                    ...
                    # Attempt predictions for Transformer as well
                    try:
                        split_idx = int(len(X) * 0.8)
                        X_test = X[split_idx:]
                        pred_payload = self.model_builder.predict_with_model("transformer", X_test)
                        self.logger.debug(f"Transformer prediction payload keys: {list(pred_payload.keys()) if isinstance(pred_payload, dict) else 'N/A'}", extra={'phase': 'MODEL_TRAINING'})
                        if isinstance(pred_payload, dict) and 'predictions' in pred_payload:
                            model_results[f'{stock}_transformer_predictions'] = pred_payload
                        elif isinstance(pred_payload, dict) and 'sample_predictions' in pred_payload:
                            model_results[f'{stock}_transformer_predictions'] = {
                                'model_name': pred_payload.get('model_name', 'transformer'),
                                'predictions': pred_payload.get('sample_predictions', [])
                            }
                    except Exception as e:
                        self.logger.warning(f"Transformer prediction failed for {stock}: {str(e)}", extra={'phase': 'MODEL_TRAINING'})
    ...
    def save_results(self, validation_results: Dict[str, Any], advanced_results: Dict[str, Any] = None):
        ...
        try:
            ...
            # Persist flat predictions into diamond_predictions if available
            try:
                if advanced_results:
                    flat_frames = []
                    exec_date = datetime.now().date()
                    for name, payload in advanced_results.items():
                        if isinstance(payload, dict) and ('predictions' in payload or 'sample_predictions' in payload):
                            arr = payload.get('predictions') if 'predictions' in payload else payload.get('sample_predictions')
                            if isinstance(arr, (list, tuple)) and len(arr) > 0:
                                df = pd.DataFrame({'pred_index': range(len(arr)), 'prediction': arr})
                                df['test_name'] = name
                                df['model_type'] = payload.get('model_name', 'Neural Network')
                                df['execution_date'] = exec_date
                                try:
                                    valeur = name.split('_')[0]
                                except Exception:
                                    valeur = None
                                df['valeur'] = valeur
                                flat_frames.append(df[['test_name','model_type','execution_date','valeur','pred_index','prediction']])
                    if flat_frames:
                        flat = pd.concat(flat_frames, ignore_index=True)
                        flat.to_sql('diamond_predictions', self.engine, if_exists='append', index=False, chunksize=500, method='multi')
                        self.logger.info(f"Saved {len(flat)} prediction rows into diamond_predictions", extra={'phase': 'RESULT_SAVING'})
            except Exception as e:
                self.logger.warning(f"Saving predictions failed: {str(e)}", extra={'phase': 'RESULT_SAVING'})
    ...
    def run_pipeline(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.enable_advanced_models:
                ...
                if model_results:
                    advanced_results.update(model_results)
    ...
    def train_model(self, model, X_train: np.ndarray, y_train: np.ndarray, 
        ...
        class LoggingCallback(tf.keras.callbacks.Callback):
            def on_epoch_end(self, epoch, logs=None):
                if (epoch + 1) % 10 == 0:
                    # Use outer class logger to avoid undefined reference
                    self.model.logger.info(f"Epoch {epoch+1}/{epochs} - loss: {logs['loss']:.6f}")
        
        # Train model
        # Attach logger on model instance for callback access
        setattr(model, 'logger', self.logger)
        history = model.fit(
            X_train, y_train,
            epochs=epochs,
